...
def main():
    ...
    with open(os.path.join(os.path.dirname(__file__), '検索キーワードリスト.txt'), encoding='utf-8') as f:  # '検索キーワードリスト.txt'ファイルを読み込み、リストにする
        keywords = [s.rstrip() for s in f.readlines()]   # １行ずつ読み込んで改行コードを削除してリストにする


    with open(os.path.join(os.path.dirname(__file__), 'ドメインリスト.txt'), encoding='utf-8') as f: # 'ドメインリスト.txt'ファイルを読み込み、リストにする
    ...
    service = ChromeService(executable_path="/Users/m/Desktop/Pythonturorial/google_weak_keywork_serch/chromedriver")
    driver = webdriver.Chrome(service=service, options=options)
...
def search(keyword, driver):
    ...
    input_element = driver.find_element(By.NAME,'q')  # 検索テキストボックスの要素をname属性から取得
...
def get_url(driver):
    ...
    objects = driver.find_elements(By.CSS_SELECTOR,'div.kb0PBd.cvP2Ce.A9Y9g.jGGQ5e > div > div > span > a')  # a要素（各ページの1位から10位までのURL）取得
